...
class MultiFileReader:
    ...
    #             self._open_files[path] = open(path, 'rb')
...
class InvertedIndex:  
    ...
    #     print(f'reading posting list for: {w}')
    #     with closing(MultiFileReader()) as reader:
    #         locs = self.posting_locs[w]
    #         b = reader.read(locs, self.df[w] * TUPLE_SIZE, path)
    #         print(f'b length: {len(b)}')
    #         posting_list = []
    #         print(f'df[{w}]: {self.df[w]}')
    #         for i in range(self.df[w]):
    #             doc_id = int.from_bytes(b[i * TUPLE_SIZE:i * TUPLE_SIZE + 4], 'big')
    ...
    def read_posting_list(self, w, path):
        with closing(MultiFileReader()) as reader:
            locs = self.posting_locs[w]
            locs = [(path +"/"+ lo[0], lo[1]) for lo in locs]
    ...
    def read_index(base_dir, name):
        print(f'reading index for {name}.pkl')
        print(f'Path(base_dir): {Path(base_dir)}')
        with open(Path(base_dir) / f'{name}.pkl', 'rb') as f:
            print('read index as f, loading as pickle....')
...
class BodyInvertedIndexExtension:
    def __init__(self):
        self.docs_num = 0
        self.avg_doc_len = 0
